...
public class LevelZDifficulty implements DifficultyGetter {
    ...
    public float getDifficulty(ServerLevel level, Vec3 pos) {
        return DifficultyGetter.getDifficulty(level, pos, p -> (float) ((PlayerStatsManagerAccess) p).getPlayerStatsManager(p).overallLevel * Config.CommonConfig.levelZScale);
    }

...
public class GeneralHelperMethods {
    ...
    public static void enchantGear(EntityMob mob)
    {
    		for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values())
        {
	            ...
	            if (itemstack != null && mob.getRNG().nextFloat() < (ConfigHandler.baseEnchantChance+(DifficultyData.get(mob.worldObj).getDifficulty()*ConfigHandler.diffEnchantAdd*0.01F)))
	            {
    ...
    {    		
    		if(mob.getRNG().nextFloat() < (ConfigHandler.baseItemChance) && mob.getRNG().nextFloat()<0.8)
    		{
        		...
        		if(stack!=null&&!itemInList(stack.getItem(), ConfigHandler.itemUseBlackList))
    ...
    public static void modifyAttr(EntityMob mob, IAttribute att, double value, double max, boolean multiply)
    {
    		...
    		value *= DifficultyData.get(mob.worldObj).getDifficulty();
    		if(multiply)
    		{

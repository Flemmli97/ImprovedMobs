...
{
    private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityMobBoat.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityMobBoat.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityMobBoat.class, DataSerializers.FLOAT);
    private static final DataParameter<Boolean>[] DATA_ID_PADDLE = new DataParameter[] {EntityDataManager.createKey(EntityMobBoat.class, DataSerializers.BOOLEAN), EntityDataManager.createKey(EntityMobBoat.class, DataSerializers.BOOLEAN)};
    ...
    private EntityMobBoat.Status status;
    private EntityMobBoat.Status previousStatus;
    ...
    {
        if (entityIn instanceof EntityMobBoat)
        {
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityMobBoat.Status.UNDER_WATER && this.status != EntityMobBoat.Status.UNDER_FLOWING_WATER)
        {
        ...
        if (this.canPassengerSteer() && ((EntityMob) this.getPassengers().get(0)).getAttackTarget()!=null)
        {
     ...
     */
    private EntityMobBoat.Status getBoatStatus()
    {
        EntityMobBoat.Status entityboat$status = this.getUnderwaterStatus();
        ...
        {
            return EntityMobBoat.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityMobBoat.Status.ON_LAND;
            }
            else
            {
                return EntityMobBoat.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityMobBoat.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)BlockLiquid.getLiquidHeight(iblockstate, this.world, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityMobBoat.Status entityboat$status = EntityMobBoat.Status.UNDER_FLOWING_WATER;
                                return entityboat$status;
                            }
        ...
        return flag ? EntityMobBoat.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityMobBoat.Status.IN_AIR && this.status != EntityMobBoat.Status.IN_AIR && this.status != EntityMobBoat.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityMobBoat.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityMobBoat.Status.IN_WATER)
            {
                d2 = (this.waterLevel - this.getEntityBoundingBox().minY) / (double)this.height;
                this.momentum = 0.9F;
            }
            else if (this.status == EntityMobBoat.Status.UNDER_FLOWING_WATER)
            {
                d1 = -7.0E-4D;
                this.momentum = 0.9F;
            }
            else if (this.status == EntityMobBoat.Status.UNDER_WATER)
            {
                d2 = 0.009999999776482582D;
                this.momentum = 0.45F;
            }
            else if (this.status == EntityMobBoat.Status.IN_AIR)
            {
                this.momentum = 0.9F;
            }
            else if (this.status == EntityMobBoat.Status.ON_LAND)
            {
    ...
    private void controlBoat()
    {
        if (this.isBeingRidden())
        {
            ...
            this.motionX += (double)(MathHelper.sin(-this.rotationYaw * 0.017453292F) * 0.05);
            this.motionZ += (double)(MathHelper.cos(this.rotationYaw * 0.017453292F) * 0.05);
    ...
    public boolean processInitialInteract(EntityPlayer player, EnumHand hand)
    {
        if (!this.world.isRemote)
        {
			player.sendMessage(new TextComponentString(TextFormatting.RED + "You can't ride a mobs boat"));
        }
    ...
    protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos)
    {
        ...
        if (!this.isRiding())
        {
            if (onGroundIn)
            {
                ...
                {
                    if (this.status != EntityMobBoat.Status.ON_LAND)
                    {

...
public class NewWalkNodeProcessor extends WalkNodeProcessor{
	...
	public int findPathOptions(PathPoint[] pathOptions, PathPoint currentPoint, PathPoint targetPoint, float maxDistance)
    {
        ...
        if (flag && flag3)
        {
			...
			AxisAlignedBB aabb1 = new AxisAlignedBB(currentPoint.xCoord-0.5 - d1 + 0.5D, currentPoint.yCoord + 1.001D, currentPoint.zCoord -0.5- d1 + 0.5D, currentPoint.xCoord-0.5 + d1 + 0.5D, ((float)currentPoint.yCoord + 1+this.entity.height), currentPoint.zCoord -0.5+ d1 + 0.5D);
			if (!this.entity.worldObj.collidesWithAnyBlock(aabb1)&&pathpoint4 != null && !pathpoint4.visited && pathpoint4.distanceTo(targetPoint) < maxDistance)
            {
        ...
        if (flag && flag2)
        {
			...
			double d1 = this.entity.width / 2.0D;
			AxisAlignedBB aabb1 = new AxisAlignedBB(currentPoint.xCoord+0.5 - d1 + 0.5D, currentPoint.yCoord + 1.001D, currentPoint.zCoord -0.5- d1 + 0.5D, currentPoint.xCoord+0.5 + d1 + 0.5D, ((float)currentPoint.yCoord + 1+this.entity.height), currentPoint.zCoord -0.5+ d1 + 0.5D);
			if (!this.entity.worldObj.collidesWithAnyBlock(aabb1) &&pathpoint5 != null && !pathpoint5.visited && pathpoint5.distanceTo(targetPoint) < maxDistance)
            {
        ...
        if (flag1 && flag3)
        {
			...
			double d1 = this.entity.width / 2.0D;
			AxisAlignedBB aabb1 = new AxisAlignedBB(currentPoint.xCoord-0.5 - d1 + 0.5D, currentPoint.yCoord + 1.001D, currentPoint.zCoord +0.5- d1 + 0.5D, currentPoint.xCoord-0.5 + d1 + 0.5D, ((float)currentPoint.yCoord + 1+this.entity.height), currentPoint.zCoord+0.5 + d1 + 0.5D);
			if (!this.entity.worldObj.collidesWithAnyBlock(aabb1) &&pathpoint6 != null && !pathpoint6.visited && pathpoint6.distanceTo(targetPoint) < maxDistance)
            {
        ...
        if (flag1 && flag2)
        {
			...
			double d1 = this.entity.width / 2.0D;
			AxisAlignedBB aabb1 = new AxisAlignedBB(currentPoint.xCoord+0.5 - d1 + 0.5D, currentPoint.yCoord + 1.001D, currentPoint.zCoord+0.5 - d1 + 0.5D, currentPoint.xCoord+0.5 + d1 + 0.5D, ((float)currentPoint.yCoord + 1+this.entity.height), currentPoint.zCoord+0.5 + d1 + 0.5D);
			if (!this.entity.worldObj.collidesWithAnyBlock(aabb1) && pathpoint7 != null && !pathpoint7.visited && pathpoint7.distanceTo(targetPoint) < maxDistance)
            {
	...
	private boolean isFence(IBlockAccess acc, IBlockState iblockstate, BlockPos blockpos, Block block)
	{
		...
		else if(this.entity!=null)
		{
			...
			iblockstate.addCollisionBoxToList(this.entity.worldObj, blockpos, collision2, list, null);
			return !list.isEmpty();
		}
		return false;
	}

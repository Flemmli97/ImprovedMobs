...
public class EntityAIBlockBreaking extends EntityAIBase{
	...
	public boolean shouldExecute() {
		...
		if(living.ticksExisted%20 == 0 && target != null && motion<1 && living.getDistanceToEntity(target) > 1D && target.onGround)
		{
			...
			IBlockState block = living.worldObj.getBlockState(blockPos);
	...
	public void updateTask() {
		if(markedLoc == null || living.worldObj.getBlockState(markedLoc).getMaterial() == Material.AIR)
		{
			digTimer = 0;
			return;
		}
		IBlockState state = living.worldObj.getBlockState(markedLoc);
		digTimer++;
		
		float str = GeneralHelperMethods.getBlockStrength(this.living, state, living.worldObj, markedLoc) * (digTimer + 1);
		if(str >= 1F)
		{
			digTimer = 0;
			...
			living.worldObj.destroyBlock(markedLoc, canHarvest);
			markedLoc = null;
		...
		} else
		{
			if(digTimer%5 == 0)
			{
				SoundType sound = state.getBlock().getSoundType(state, living.worldObj, markedLoc, living);
				living.getNavigator().setPath(living.getNavigator().getPathToPos(markedLoc), 1D);
				living.worldObj.playSound(null, markedLoc, ConfigHandler.useBlockBreakSound? sound.getBreakSound():SoundEvents.BLOCK_NOTE_BASS, SoundCategory.BLOCKS, 2F, 0.5F);
				...
				living.worldObj.sendBlockBreakProgress(living.getEntityId(), markedLoc, (int)(str)*digTimer*10);
			}
		}
	}

package com.flemmli97.improvedmobs.entity.ai;

import net.minecraft.block.state.IBlockState;
import net.minecraft.entity.EntityLiving;
import net.minecraft.entity.ai.EntityAIBase;
import net.minecraft.pathfinding.Path;
import net.minecraft.pathfinding.PathPoint;

public class EntityAIClimbLadder extends EntityAIBase{

	EntityLiving living;	
	private Path path;
	public EntityAIClimbLadder(EntityLiving living)
	{
		this.living=living;
		this.setMutexBits(4);
	}
	@Override
	public boolean shouldExecute() {
		if(!this.living.getNavigator().noPath())
		{
			this.path= this.living.getNavigator().getPath();
			return this.living.isOnLadder();
		}
		return false;
	}
	
	public void updateTask()
	{
		int i = this.path.getCurrentPathIndex();
		if(i+1<this.path.getCurrentPathLength())
		{
			int y = this.path.getPathPointFromIndex(i).yCoord;//this.living.getPosition().getY();
			PathPoint pointNext = this.path.getPathPointFromIndex(i+1);
			IBlockState down = this.living.worldObj.getBlockState(this.living.getPosition().down());
			if(pointNext.yCoord<y||(pointNext.yCoord==y &&!down.getBlock().isLadder(down, this.living.worldObj, this.living.getPosition().down(), this.living)))
				this.living.motionY=-0.15;
			else
				this.living.motionY=0.15;
		}
	}
}

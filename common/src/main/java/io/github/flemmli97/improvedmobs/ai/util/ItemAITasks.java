...
public class ItemAITasks {
    ...
    private static boolean blockedAI(Mob entity, Item item) {
        return (Config.CommonConfig.mobListUseWhitelist && !Config.CommonConfig.itemuseBlacklist.contains(RegistryHelper.items().getIDFrom(item).toString()))
                || Config.CommonConfig.itemuseBlacklist.contains(RegistryHelper.items().getIDFrom(item).toString())
                || Config.CommonConfig.entityItemConfig.preventUse(entity, item);
    }

    private static void initVanilla() {
        for (Item item : RegistryHelper.items().getIterator()) {

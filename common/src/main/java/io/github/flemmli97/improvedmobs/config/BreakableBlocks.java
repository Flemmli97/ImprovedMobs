...
public class BreakableBlocks implements IConfigListValue<BreakableBlocks> {
    ...
    private static void addBlocks(String s, Set<String> blocks) {
        if (s.startsWith("#")) {
            Iterable<Holder<Block>> tag = Registry.BLOCK.getTagOrEmpty(TagKey.create(Registry.BLOCK_REGISTRY, new ResourceLocation(s.substring(1))));
            tag.forEach(h -> blocks.add(Registry.BLOCK.getKey(h.value()).toString()));
        } else
            blocks.add(s);
    }
